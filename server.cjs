...
var dbState = {
  ...
  products: [
    ...
    },
    {
      ...
      discountPrice: 249,
      quantity: 3,
      // Very low stock to trigger warnings!
      images: [
    ...
    {
      id: "prod-furn-1",
      name: "Malm\xF6 Ergonomic Office Chair with Flex-Lumbar",
    ...
    },
    {
      ...
      specifications: {
        ...
        "CADR Rating": "320 m\xB3/h volumetric output flow",
...
async function startServer() {
  ...
  app.post("/api/auth/google-register", (req, res) => {
    ...
    const newUser = {
      ...
      mobile: "+91 " + Math.floor(9e9 + Math.random() * 999999999),
      role: "Customer",
      gender: "Other",
      dob: "1999-01-01",
  ...
  app.post("/api/products", (req, res) => {
    ...
    const newProduct = {
      ...
      brand: data.brand,
      sku: data.sku || "SKU-" + Math.floor(Math.random() * 9e5),
      price: Number(data.price),
      discountPrice: data.discountPrice ? Number(data.discountPrice) : void 0,
    ...
    const newNotif = {
      ...
      title: "New Product Arrival! \u{1F680}",
  ...
  app.post("/api/orders", (req, res) => {
    ...
    const orderId = "SMC-2026-" + Math.floor(1e3 + Math.random() * 9e3);
    const newOrder = {
      id: orderId,
      date: (/* @__PURE__ */ new Date()).toISOString(),
      status: "Confirmed",
...
*Heuristic Help*: I am currently stocked with the **Quantum 5G Smartphone**, **VocalNoise Suite Headset**, and **Aero-Pace Runner shoes** in our active catalogue.`
  ...
  app.get("/api/admin/reports", (req, res) => {
    ...
    const revenue = dbState.orders.filter((o) => o.status !== "Cancelled").reduce((acc, o) => acc + o.payableAmount, 0);
